...
static PyObject* compiled_func(PyObject*self, PyObject* args)
{
    ...
    PyObject *py_arr_blitz_not_buggy, *py_sh, *py_arr;
    py_arr_blitz_not_buggy = py_sh = py_arr = NULL;
    ...
    py_arr_blitz_not_buggy = get_variable("arr_blitz_not_buggy",raw_locals,raw_globals);
    PyArrayObject* arr_blitz_not_buggy_array = convert_to_numpy(py_arr_blitz_not_buggy,"arr_blitz_not_buggy");
    conversion_numpy_check_type(arr_blitz_not_buggy_array,PyArray_DOUBLE,"arr_blitz_not_buggy");
    conversion_numpy_check_size(arr_blitz_not_buggy_array,1,"arr_blitz_not_buggy");
    blitz::Array<double,1> arr_blitz_not_buggy = convert_to_blitz<double,1>(arr_blitz_not_buggy_array,"arr_blitz_not_buggy");
    blitz::TinyVector<int,1> Narr_blitz_not_buggy = arr_blitz_not_buggy.shape();
    py_sh = get_variable("sh",raw_locals,raw_globals);
    int sh = convert_to_int(py_sh,"sh");
    py_arr = get_variable("arr",raw_locals,raw_globals);
    PyArrayObject* arr_array = convert_to_numpy(py_arr,"arr");
    conversion_numpy_check_type(arr_array,PyArray_DOUBLE,"arr");
    conversion_numpy_check_size(arr_array,1,"arr");
    blitz::Array<double,1> arr = convert_to_blitz<double,1>(arr_array,"arr");
    blitz::TinyVector<int,1> Narr = arr.shape();
        /* inline code */                   
    /* NDARRAY API VERSION 1000009 */
    arr_blitz_not_buggy(blitz::Range(1,sh-1))=arr(blitz::Range(1,_end));
    ...
    /* cleanup code */                   
    Py_XDECREF(py_arr_blitz_not_buggy);
    Py_XDECREF(py_arr);
...
PyMODINIT_FUNC initsc_e0302caa199e71ff4f7c6e6dbb59d3a26cff42f665cef435854e25451e8892c50(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("sc_e0302caa199e71ff4f7c6e6dbb59d3a26cff42f665cef435854e25451e8892c50", compiled_methods);
}
